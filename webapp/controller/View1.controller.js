sap.ui.define([
    ...
    "sap/btp/wizard/controller/BaseController",
    ...
    */
    function (JSONModel, ODataModel, DateFormatter, NumberFormatter, 
        ...
        return BaseController.extend("sap.btp.wizard.controller.View1", 
        {
            ...
            onNextStep: async function() {
                ...
                else if (oProgressStep === this.byId("step2")) 
                {
                ...
                else if (oProgressStep === this.byId("step3")) 
                {
                ...
                // Imposta la visibilità dei pulsanti per lo step successivo
                var oNextStep = oWizard.getProgressStep();

                if(oNextStep === this.byId("step1")) 
                {
                ...
                else if(oNextStep === this.byId("step2")) 
                {
                    oVisibilityModel.setProperty("/nextButtonVisible", true);
                    oVisibilityModel.setProperty("/backButtonVisible", true);
                    oVisibilityModel.setProperty("/confirmButtonVisible", false);
                } 
                else if(oNextStep === this.byId("step3")) 
                {
                    oVisibilityModel.setProperty("/nextButtonVisible", true);
                    oVisibilityModel.setProperty("/backButtonVisible", true);
                    oVisibilityModel.setProperty("/confirmButtonVisible", false);
                }
                else if(oNextStep === this.byId("step4")) 
                {
                    oVisibilityModel.setProperty("/nextButtonVisible", false);
                    oVisibilityModel.setProperty("/backButtonVisible", true);
                    oVisibilityModel.setProperty("/confirmButtonVisible", true);
                }
            },
            ...
            onPreviousStep: function() {
                ...
                if (oCurrentStep === this.byId("step1")) 
                {
                    oVisibilityModel.setProperty("/backButtonVisible", false);
                } 
                else if (oCurrentStep === this.byId("step4")) 
                {
                    oVisibilityModel.setProperty("/confirmButtonVisible", false);
                    oVisibilityModel.setProperty("/nextButtonVisible", true);
                } 
            ...
            onCustomerSelectionChange: function(oEvent) {
                ...
                // Ottieni il fragment dello Step4
                var oStep4Fragment = this.byId("step4");
                ...
                MessageBox.confirm("Confermare il suo ordine?", {
                    ...
                    onClose: function(oAction) {
                        if (oAction === MessageBox.Action.OK) 
                        {
                            ...
                            // Dopo aver confermato l'ordine e tornato allo Step 1
                            // Torna all'inizio del wizard utilizzando previousStep
                            while (that.byId("myWizard").getProgressStep() !== that.byId("step1")) {
                                that.byId("myWizard").previousStep();
                            }
                        }
                    }
                });
